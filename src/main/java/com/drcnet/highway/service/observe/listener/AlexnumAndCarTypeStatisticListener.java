...
/**
 * @Author jack
 * @Date: 2019/8/13 13:18
 * @Desc:
 **/
@Component
@Slf4j
public class AlexnumAndCarTypeStatisticListener extends BaseListener implements SmartApplicationListener {
    @Resource
    ...
    public void doWork(Integer maxTietouId, Integer maxCarDicId, Integer previousTietouId, Integer previousCarId, Integer previousEndMonth, Integer taskId) {
        System.out.println("this is no.2 step...");
        if (maxCarDicId > previousCarId) {
            tietouCleanService.statisticsAlexNumAndCarType(previousCarId, maxCarDicId);
            updateTaskState(taskId, CalculateStateEnum.ALEXNUM_AND_CARTYPE_STATISTIC.getCode());
        } else {
            updateTaskState(taskId, CalculateStateEnum.ALEXNUM_AND_CARTYPE_STATISTIC.getCode());
            log.info("------------AlexnumAndCarTypeStatisticListener 执行成功! 当前car_dic表最大id为：{}，前次执行的最大id为：{}，无新数据，本次不执行新计算！", maxCarDicId, previousCarId);
        }
    }
    ...
    @Override
    public int getOrder() {
        return CalculateStateEnum.ALEXNUM_AND_CARTYPE_STATISTIC.getCode();
    }
}
